...
In this case, the `wards` data are in an appropriate system but the `race` data need to be transformed. The `st_transform()` function can be used to modify them to an appropriate coordinate sysmtem:
...
Another option would be to rename the column using the `rename()` function from `dplyr`:
...
Either option will work for `areal`'s purposes.
